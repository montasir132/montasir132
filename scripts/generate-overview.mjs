...
for (const repo of repos.slice(0, 40)) {
  ...
  try {
    ...
    for (const [language, bytes] of Object.entries(langData)) {
      languageCounts[language] =
        (languageCounts[language] || 0) + bytes;
    }
  ...
  try {
    ...
    repoCommitCounts[repo.language || "Other"] =
      (repoCommitCounts[repo.language || "Other"] || 0) +
      Math.min(commits.length, 1);
...
const points = recentWeekly.map((value, i) => {
  ...
  const x =
    graphX +
    (i / Math.max(recentWeekly.length - 1, 1)) *
      graphW;

  const y =
    graphY +
    graphH -
    (value / maxWeekly) * graphH;
...
/*
  ...
  Previously the labels were placed outside the card:

  graphX + graphW + 12

  Now they are placed INSIDE the graph area.
*/
...
>
  ...
  <text
    ...
    class="name"
  >
    ${esc(shortName(user.name || username))}
  </text>
